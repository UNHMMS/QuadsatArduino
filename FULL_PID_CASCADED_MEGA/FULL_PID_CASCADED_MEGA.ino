...
PID rollCtrl( &curAtt[0], &attOut[0], &desAtt[0], .9, 0, .002, DIRECT );
PID pitchCtrl(&curAtt[1], &attOut[1], &desAtt[1], .9, 0, .002, DIRECT );
...
PID roll_R_Ctrl( &curGyro[0], &rateOut[0], &attOut[0], .1, .05, .02, DIRECT );
PID pitch_R_Ctrl(&curGyro[1], &rateOut[1], &attOut[1], .1, .05, .02, DIRECT );
PID yaw_R_Ctrl(  &curGyro[2], &rateOut[2], &zero, .1, .05, .02, DIRECT );
  ...
  */
  ...
  for( int i = 0; i < 4; i++ )
  {
    ...
    */
    ...
    motorOutput[i] = (motorOutput[i] / SLOPEVAL) + 6;
  }
