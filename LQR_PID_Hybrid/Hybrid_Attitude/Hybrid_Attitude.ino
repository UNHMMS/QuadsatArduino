...
#define ZMIN (10)
#define ZMAX (32)          //the maximum value (in newtons) that the motors can push
...
double desZ = 0.3; //desired altitude
...
PID pitchCtrl(&curAngles[0], &attOut[0], &zero, 4.5, 12, .2, DIRECT );
PID rollCtrl (&curAngles[1], &attOut[1], &zero, 4.5, 12, .2, DIRECT );
PID yawCtrl  (&curAngles[2], &attOut[2], &zero, 4.5, 12, .2, DIRECT );
PID zPosCtrl( &curZ, &outZ, &desZ, 12, 5, .25, DIRECT );
