...
float lastCompAngle[3] = { 0.0, 0.0, 0.0 };
...
void updateIMU()
{
  ...
  */
  ...
  rawGyro[0] = lastCompAngle[0] + gyroFlt[0] * timeStep;
  rawGyro[1] = lastCompAngle[1] + gyroFlt[1] * timeStep;
  
  filterAngle[0] = COMPRATIO * ( rawGyro[0] ) + (1 - COMPRATIO) * axlAngle[1];
  filterAngle[1] = COMPRATIO * ( rawGyro[1] ) + (1 - COMPRATIO) * axlAngle[0];
  filterAngle[2] = COMPRATIO * ( lastCompAngle[2] + gyroFlt[2] * timeStep ) + (1 - COMPRATIO) * rawHeading;
  
  for( int i = 0; i < 3; i++ )
  {
    lastCompAngle[i] = filterAngle[i];
  }
  
  /*
  filterAngle[0] = kFilterRoll.getAngle(axlAngle[1], gyroFlt[0], timeStep);
  filterAngle[1] = kFilterPitch.getAngle(axlAngle[0], gyroFlt[1], timeStep);
  filterAngle[2] = kFilterYaw.getAngle(rawHeading, gyroFlt[2], timeStep);
  */
...
float getRawGyroPitch()
{
  return rawGyro[1];
}

float getRawGyroRoll()
{
  return rawGyro[0];
}
...
float getRoll()
{
  return filterAngle[0];
}

float getPitch()
{
  return filterAngle[1];
}

float getYaw()
{
  return filterAngle[2]; 
}

void setInitialKalmanAngles()
{
  kFilterRoll.setAngle( axlAngle[1] );
  kFilterPitch.setAngle( axlAngle[0] );
}

void setInitialComplementaryAngles()
{
  lastCompAngle[0] = axlAngle[1];
  lastCompAngle[1] = axlAngle[0];
  lastCompAngle[2] = rawHeading;
}
