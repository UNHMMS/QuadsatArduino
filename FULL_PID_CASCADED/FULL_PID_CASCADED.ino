...
#define MINPOT (4)         //the minimum valid potentiometer value for the motors
...
PID rollCtrl( &curAtt[0], &attOut[0], &desAtt[0], .5, 0, .05, DIRECT );
PID pitchCtrl(&curAtt[1], &attOut[1], &desAtt[1], .5, 0, .05, DIRECT );
//PID yawCtrl(  &curAtt[2], &attOut[2], &desAtt[2], 0, 0, 0, DIRECT );
PID zPosCtrl( &curAtt[3], &attOut[3], &desAtt[3], 3, 1, 3, DIRECT );
/*
...
PID roll_R_Ctrl( &curGyro[0], &rateOut[0], &attOut[0], .05, .25, .01, DIRECT );
PID pitch_R_Ctrl(&curGyro[1], &rateOut[1], &attOut[1], .05, .25, .01, DIRECT );
PID yaw_R_Ctrl(  &curGyro[2], &rateOut[2], &zero, .05, .25, .01, DIRECT );
...
void loop()
{
     ...
     if( ctrlOn )
    {
      ...
      */
      ...
      if( ((micros() - loopStart) > 500000) )
      {
        //zPosCtrl.SetOutputLimits( 0, 16 );
